...
class InitServiceWrapperPathTests(unittest.TestCase):
    ...
    def test_init_switches_to_sft_and_4b_when_available(
        ...
        """Gradio init should switch to SFT + 4B when those models exist."""
        module = self._import_module()
        ...
        mock_ensure_main_model.assert_called_once()
        init_args = dit_handler.initialize_service.call_args[0]
        self.assertEqual(init_args[1], "acestep-v15-sft")

        _, llm_kwargs = llm_handler.initialize.call_args
        self.assertEqual(llm_kwargs.get("lm_model_path"), "acestep-5Hz-lm-4B")
